...
#if MQTT_USE_TLS != 0
#error "The RTK dashboard broker contract is plaintext MQTT; set MQTT_USE_TLS to 0"
#endif
...
namespace
{
  ...
  tide::MqttTelemetry *mqttTelemetry = nullptr;
  ...
  {
    if (!telemetryUsesBlynk() || !Blynk.connected())
    {
      return;
    }
  ...
  void armMqttUploadDeadline()
  {
    ...
    mqttUploadDeadlineArmed = true;
    mqttUploadDeadlineStartedAt = millis();
  }

  bool mqttUploadDeadlineExpired()
  {
    return mqttUploadDeadlineArmed && !cycleUploaded &&
           millis() - mqttUploadDeadlineStartedAt >= MQTT_UPLOAD_DEADLINE_MS;
  }

  bool buildAndPublishMqttRecord(const tide::MeasurementRecord &record)
  {
    char payload[tide::TELEMETRY_PAYLOAD_CAPACITY] = {};
    size_t payloadLength = 0;
    const tide::TelemetryPayloadContext context = {
        mqttTelemetry->deviceId(),
        offlineQueue.ready() ? offlineQueue.count() : 0,
        offlineQueue.ready() ? offlineQueue.droppedRecords() : 0,
        BLYNK_FIRMWARE_VERSION,
        WiFi.RSSI(),
        millis() / 1000UL,
    };
    if (!tide::serializeTelemetryPayload(record, context, payload,
                                         sizeof(payload), payloadLength))
    {
      Serial.printf("ERROR MQTT: JSON sequence=%lu melebihi/invalid buffer %u.\n",
                    static_cast<unsigned long>(record.sequence),
  ...
  void serviceMqttOfflineUpload()
  {
    ...
    if (mqttTelemetry->takeAcknowledgedSequence(acknowledgedSequence))
    {
      if (!currentCycleRecordQueued || !offlineQueue.ready())
      {
        if (acknowledgedSequence != currentCycleRecord.sequence)
        {
          Serial.printf("MQTT PUBACK sequence=%lu tidak cocok dengan record RAM; "
                        "diabaikan.\n",
    ...
    if (!offlineQueue.ready() || !currentCycleRecordQueued)
    {
      if (currentCycleRecord.sequence == 0)
      {
        Serial.println("ERROR MQTT: record RAM tanpa sequence durable tidak "
                       "dikirim; menunggu deadline agar tidak mengaku berhasil.");
        return;
      }
      buildAndPublishMqttRecord(currentCycleRecord);
      return;
    }
  ...
  void performOtherCycleMeasurements()
  {
    ...
    if (solarVoltageValid)
      Serial.printf("%.3f V", latestSolarVoltage);
    Serial.println();
    Serial.printf("Baterai CH2: %s", batteryValid ? "" : "ERROR ");
    if (batteryValid)
      Serial.printf("%.3f V", latestBatteryVoltage);
    Serial.println();
    if (system5VVoltageValid)
    {
  ...
  void enterTimedDeepSleep()
  {
    ...
    {
      mqttTelemetry->disconnectForSleep();
      const uint32_t disconnectStartedAt = millis();
      while (!mqttTelemetry->disconnected() &&
             millis() - disconnectStartedAt < MQTT_DISCONNECT_GRACE_MS)
      {
        delay(1); // Yield to the MQTT task and watchdog; strictly bounded.
      }
    }
    Blynk.disconnect();
...
void loop()
{
  ...
  if (BlynkState::is(MODE_ERROR))
  {
    ...
    if (telemetryUsesMqtt() && WiFi.status() == WL_CONNECTED &&
        !cycleUploaded && !mqttUploadDeadlineExpired())
    {
      // Blynk Cloud failure must not cut short the independent local MQTT
      // attempt. serviceMqttOfflineUpload() already ran near the top.
      return;
    }
    abortEdgentConnectionForThisCycle();
    enterTimedDeepSleep();
